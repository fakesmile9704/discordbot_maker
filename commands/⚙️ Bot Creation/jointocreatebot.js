...
module.exports = {
    ...
    description: "Cada servidor necesita un bot Unirse para crear, que crea **canales temporales** que se **eliminan** una vez que se vuelven **vacíos**",
                        ...
                        setTimeout(()=>{
                          ...
                          tempmsg.edit(new MessageEmbed()
                            ...
                            .setTitle(`¿Cómo usar el Bot?`)
                             .setDescription(`1. Descargue el archivo ZIP\n2. Extraiga el ARCHIVO ZIP en una CARPETA\n3. ¡abra una nueva TERMINAL (cmd/powershell) en este Directorio!\n4. escriba: \`npm install\` para instalar todo paquetes necesarios (discord.js ascii-table colors)\n5. Después de eso, escriba \`node index.js\` y su Bot se iniciará.\n\n\nAhora invite a su Bot a su servidor deseado y escriba \`$ {prefix}addchannel <ChannelID>\` para agregar un canal de voz a un Join para crear la configuración del canal.\nPuede tener múltiples Join para crear configuraciones del canal a la vez\nPara eliminar uno, simplemente escriba: \`${prefix}removechannel <ChannelID >\`\n\nPara ver todos los demás comandos disponibles, escriba: \`${prefix}help\`\n*¡Tenga en cuenta que este bot solo funciona en un gremio a la vez!*\n\nTambién puede cambiar los colores **Embed** en \`/botconfig/embed.json\``)
                           )
                          setTimeout(()=>{
                            try {
